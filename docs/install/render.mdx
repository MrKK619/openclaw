...
services:
  - type: web
    ...
    envVars:
      ...
      - key: OPENCLAW_ALLOWED_ORIGINS
        value: "" # set your public URL here (e.g. https://your-service.onrender.com)
      - key: OPENCLAW_DANGEROUS_ALLOW_HOST_HEADER_ORIGIN_FALLBACK
        value: "false"
    disk:
      name: openclaw-data
      mountPath: /data
...
### Allowed Origins and Security

When deploying on non-loopback network interfaces like Render, the gateway strictly rejects browser WebSocket connections from unauthorized origins to prevent hijacking.

To authorize the Control UI to connect, set the allowed browser origins:

1. **Allowed Origins**: Set the `OPENCLAW_ALLOWED_ORIGINS` environment variable in your service dashboard to your public domain (e.g. `https://your-service.onrender.com`). If you are running a local dashboard, you can also include local origins (e.g. `https://your-service.onrender.com,http://localhost:5173`).
2. **Alternative (Host-Header Fallback)**: Alternatively, you can set `OPENCLAW_DANGEROUS_ALLOW_HOST_HEADER_ORIGIN_FALLBACK=true` to dynamically allow any origin matching the gateway's Host header. _Use with care, as this weakens DNS rebinding and hijacking protection._
